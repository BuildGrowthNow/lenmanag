...
async def refresh_session(request: Request, response: Response, session_cookie: str | None = Cookie(default=None, alias=SESSION_COOKIE_NAME)) -> ResponseEnvelope[SessionResponse]:
    if not session_cookie:
        raise HTTPException(status_code=401, detail="Authentication required.")
    payload = decode_session_token(session_cookie)
    if payload is None:
        raise HTTPException(status_code=401, detail="Authentication required.")
    token = create_session_token(email=payload["email"], name=payload.get("name"), role=payload.get("role", "operator"))
    _set_session_cookie(response, token)
...
async def logout(request: Request, response: Response, session_cookie: str | None = Cookie(default=None, alias=SESSION_COOKIE_NAME)) -> ResponseEnvelope[LogoutResponse]:
    ...
    return success_response(LogoutResponse(status="logged_out"), meta=response_meta(request))
