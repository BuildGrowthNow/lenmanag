...
def _brief_asset_provenance(asset_cues: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return _unique_by_key([_asset_reference_from_cue(cue) for cue in asset_cues], _brief_reference_key)
...
def _lead_doc_to_detail(doc: dict[str, Any], jobs: list[dict[str, Any]] | None = None) -> LeadDetail:
    latest_job = None
    if jobs:
...
def _lead_doc_to_list_item(doc: dict[str, Any], latest_job: dict[str, Any] | None = None) -> LeadListItem:
    return LeadListItem(
        id=str(doc["id"]),
        sourceType=doc["sourceType"],
        companyName=doc.get("companyName"),
...
def _build_lead_doc(
    *,
    ...
    lead = {
        ...
        "industry": industry,
        "notes": notes,
        "missingFields": _missing_fields({"companyName": company_name, "websiteUrl": website_url}),
        "version": 1,
        "latestJobId": None,
...
def _merge_lead_doc(existing: dict[str, Any], incoming: dict[str, Any]) -> dict[str, Any]:
    merged = dict(existing)
    merged["sourceRefs"] = list(existing.get("sourceRefs", [])) + list(incoming.get("sourceRefs", []))
    ...
    merged["status"] = "needs_review" if merged["missingFields"] else (merged.get("status") or "new")
    ...
    merged["latestExtractionId"] = incoming.get("latestExtractionId", existing.get("latestExtractionId"))
    merged["jobIds"] = list(dict.fromkeys(list(existing.get("jobIds", [])) + list(incoming.get("jobIds", []))))
    return merged
...
class LeadRepository:
    ...
    async def _maybe_ensure_indexes(self) -> None:
        ...
        await database["site_extractions"].create_index([("leadId", 1), ("version", -1)])
        ...
        if database is None:
            async with self._memory_lock:
                ...
                if existing is not None:
                    ...
                    return LeadActionResponse(lead=lead, created=False, merged=True, jobId=None, message="Lead merged into existing record.")
                self._memory[incoming["id"]] = incoming
                return LeadActionResponse(lead=_lead_doc_to_detail(incoming), created=True, merged=False, jobId=None, message="Lead created.")
    ...
    def _find_duplicate_memory(self, normalized_domain: str) -> dict[str, Any] | None:
        for lead in self._memory.values():
            if lead.get("normalizedDomain") == normalized_domain and lead.get("status") != "archived":
                return lead
        return None

    async def import_csv(self, *, file_name: str | None, csv_bytes: bytes) -> LeadImportResponse:
        await self._maybe_ensure_indexes()
        ...
        for index, row in enumerate(rows, start=1):
            try:
                lead, created, merged, message = await self._import_row(row, source_ref=f"{file_name or 'csv'}:row:{index}")
                if lead:
                ...
                items.append(
                    ImportRowResult(
                        rowNumber=index,
                        status="created" if created else ("merged" if merged else "failed"),
                        leadId=lead["id"] if lead else None,
                        companyName=lead.get("companyName") if lead else row.get("companyName"),
                        websiteUrl=lead.get("websiteUrl") if lead else row.get("websiteUrl") or row.get("url"),
                        normalizedDomain=lead.get("normalizedDomain") if lead else None,
                        message=message,
                        missingFields=list(lead.get("missingFields", [])) if lead else [],
                    )
                )
    ...
    async def _import_row(self, row: dict[str, str], *, source_ref: str | None) -> tuple[dict[str, Any] | None, bool, bool, str]:
        company_name = self._read_column(row, ["companyName", "company", "name"])
        website_value = self._read_column(row, ["websiteUrl", "website", "url", "domain"])
        ...
        if database is None:
            async with self._memory_lock:
                ...
                if existing is not None:
                    merged = _merge_lead_doc(existing, incoming)
                    self._memory[existing["id"]] = merged
                    return merged, False, True, "Merged into existing lead."
                self._memory[incoming["id"]] = incoming
                return incoming, True, False, "Lead created."

        existing = await database["leads"].find_one({"normalizedDomain": normalized_domain, "status": {"$ne": "archived"}})
        if existing:
            merged = _merge_lead_doc(existing, incoming)
            await database["leads"].replace_one({"id": existing["id"]}, merged)
            return merged, False, True, "Merged into existing lead."
    ...
    def _read_column(self, row: dict[str, str], candidates: Iterable[str]) -> Optional[str]:
    ...
    async def list_leads(self, q: str | None = None, status: str | None = None, limit: int = 25, offset: int = 0) -> LeadListResponse:
        await self._maybe_ensure_indexes()
        database = get_database()
        if database is None:
            async with self._memory_lock:
                items = self._list_memory_leads(q=q, status=status, limit=limit, offset=offset)
                total = self._count_memory_leads(q=q, status=status)
                return LeadListResponse(items=items, pagination={"total": total, "limit": limit, "offset": offset})
        ...
        cursor = database["leads"].find(query).sort("updatedAt", -1).skip(offset).limit(limit)
    ...
    def _list_memory_leads(self, q: str | None, status: str | None, limit: int, offset: int) -> list[LeadListItem]:
        ...
        return [_lead_doc_to_list_item(doc, self._latest_job_memory(doc.get("latestJobId"))) for doc in sliced]
    ...
    async def _lead_list_item_for_doc(self, doc: dict[str, Any], database) -> LeadListItem:
        latest_job = None
    ...
    async def get_lead(self, lead_id: str) -> LeadDetail | None:
        ...
        if doc is None:
            return None
        jobs_cursor = database["jobs"].find({"leadIds": lead_id}).sort("updatedAt", -1).limit(10)
    ...
    async def update_lead(self, lead_id: str, patch: LeadPatchRequest) -> LeadDetail | None:
        await self._maybe_ensure_indexes()
        database = get_database()
        if database is None:
            async with self._memory_lock:
    ...
    def _apply_patch(self, doc: dict[str, Any], patch: LeadPatchRequest) -> dict[str, Any]:
        updated = dict(doc)
        ...
        if updated["status"] != "archived":
            updated["status"] = "needs_review" if updated["missingFields"] else updated.get("status", "new")
        ...
        *,
            ...
            async with self._memory_lock:
                ...
                for lead_id in lead_ids:
                    ...
                    if lead is not None:
                        ...
                        lead["jobIds"] = list(dict.fromkeys(list(lead.get("jobIds", [])) + [doc["id"]]))
                        lead["updatedAt"] = now
        ...
        for lead_id in lead_ids:
            await database["leads"].update_one(
                {"id": lead_id},
                {"$set": {"latestJobId": doc["id"], "updatedAt": now}, "$addToSet": {"jobIds": doc["id"]}},
            )
        return _job_doc_to_summary(doc)
    ...
    async def _update_job(
        ...
        *,
                ...
                if lead_ids is not None:
                    ...
                    for lead_id in lead_ids:
                        ...
                        if lead is not None:
                            ...
                            lead["jobIds"] = list(dict.fromkeys(list(lead.get("jobIds", [])) + [job_id]))
                            lead["updatedAt"] = now
                return
        ...
        if lead_ids is not None:
            await database["jobs"].update_one({"id": job_id}, {"$set": {"leadIds": lead_ids, "leadId": lead_ids[0] if len(lead_ids) == 1 else None}})
            for lead_id in lead_ids:
                await database["leads"].update_one(
                    {"id": lead_id},
                    {"$set": {"latestJobId": job_id, "updatedAt": now}, "$addToSet": {"jobIds": job_id}},
                )
    ...
    async def retry_job(self, job_id: str, *, request: JobRetryRequest | None = None) -> JobSummary | None:
        await self._maybe_ensure_indexes()
        ...
        await self._update_job(
            ...
            metadata={**metadata, "retryOfJobId": source["id"], "retryCount": retry_count},
        )
        database = get_database()
        if database is None:
            async with self._memory_lock:
        ...
        await database["jobs"].update_one(
            {"id": job.id},
            {"$set": {"retryOfJobId": source["id"], "retryCount": retry_count, "status": "queued", "progress": 0, "step": "Queued for retry"}},
        )
    ...
    async def get_queue_health(self) -> JobQueueHealthResponse:
        ...
        for doc in docs:
            by_type[doc.get("jobType", "unknown")] = by_type.get(doc.get("jobType", "unknown"), 0) + 1
    ...
    async def search_jobs_for_lead(self, lead_id: str) -> list[JobSummary]:
        ...
        if database is None:
            async with self._memory_lock:
                return [_job_doc_to_summary(job) for job in self._jobs_for_lead_memory(lead_id)]
        cursor = database["jobs"].find({"leadIds": lead_id}).sort("updatedAt", -1).limit(10)
    ...
    async def _latest_extraction_doc(self, lead_id: str) -> dict[str, Any] | None:
        ...
        return await database["site_extractions"].find_one({"leadId": lead_id}, sort=[("version", -1)])
    ...
    def _extraction_doc_to_snapshot(self, doc: dict[str, Any]) -> ExtractionSnapshot:
        return ExtractionSnapshot(
            ...
            detectedWebsiteUrl=doc.get("detectedWebsiteUrl"),
            summary=doc["summary"],
            pageInventory=list(doc.get("pageInventory", [])),
    ...
    async def _latest_brief_doc(self, lead_id: str) -> dict[str, Any] | None:
        ...
        return await database["site_briefs"].find_one({"leadId": lead_id}, sort=[("version", -1)])

    def _brief_doc_to_snapshot(self, doc: dict[str, Any]) -> SiteBrief:
        company_summary = doc.get("companySummary")
        value_proposition_summary = doc.get("valuePropositionSummary") or company_summary
        return SiteBrief(
            id=str(doc["id"]),
            leadId=str(doc["leadId"]),
    ...
    def _brief_source_references(
        ...
        *,
        ...
        refs = [_page_reference_from_citation(citation) for citation in extraction.sourceCitations]
        ...
        *,
        ...
        if evidence_types:
            refs.extend(
                ...
                if citation.get("evidenceType") in evidence_types
            )
        else:
            refs.extend(_page_reference_from_citation(citation) for citation in extraction.sourceCitations)
        if include_assets:
            refs.extend(_asset_reference_from_cue(cue) for cue in asset_cues)
    ...
    def _build_brief_doc(
        ...
        *,
        ...
        now = _now()
        asset_cues = [cue.model_dump() if hasattr(cue, "model_dump") else cue for cue in extraction.brandAssetCues]
        source_refs = self._brief_source_references(extraction=extraction, asset_cues=asset_cues)
        previous_version = int(previous_brief.get("version", 0)) if previous_brief else 0
        next_version = previous_version + 1

        positioning = extraction.summary.positioningSummary
        company_summary_value = positioning or (
            f"{lead.companyName} is the lead record for {lead.websiteUrl}." if lead.companyName else f"Public positioning for {lead.websiteUrl} is sparse in the current crawl."
        )
        company_refs = self._field_references(extraction=extraction, asset_cues=asset_cues, evidence_types=["title", "meta"], limit=2)
        company_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in company_refs), floor=38)
        company_source_kind: BriefSourceKind = "source_backed" if positioning else "inferred"
        company_inference = (
        ...
        value_proposition_signal = extraction.summary.positioningSummary or (extraction.summary.serviceClues[0] if extraction.summary.serviceClues else "")
        ...
        value_proposition_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in value_proposition_refs), floor=35)

        audience_signal = ", ".join(extraction.summary.audienceClues[:3]) if extraction.summary.audienceClues else ""
        audience_value = (
        ...
        audience_refs = self._field_references(extraction=extraction, asset_cues=asset_cues, evidence_types=["heading", "meta", "title"], limit=2)
        audience_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in audience_refs), floor=32)

        tone_signal = ", ".join(extraction.summary.toneClues[:3]) if extraction.summary.toneClues else ""
        ...
        tone_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in tone_refs), floor=30)

        conversion_signal = extraction.summary.ctaClues[0] if extraction.summary.ctaClues else (extraction.summary.serviceClues[0] if extraction.summary.serviceClues else "")
        conversion_value = (
        ...
        conversion_refs = self._field_references(extraction=extraction, asset_cues=asset_cues, evidence_types=["cta", "heading"], limit=2)
        conversion_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in conversion_refs), floor=34)
        ...
        hero_refs = self._field_references(extraction=extraction, asset_cues=asset_cues, evidence_types=["title", "heading", "meta"], include_assets=True, limit=3)
        hero_confidence = self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in hero_refs), floor=28)

        section_refs = self._field_references(extraction=extraction, asset_cues=asset_cues, evidence_types=["heading", "cta", "title"], include_assets=True, limit=3)
        section_items: list[dict[str, Any]] = [
            _brief_section_recommendation(
                ...
                confidence=self._brief_confidence(extraction.confidenceScore, *(ref["confidence"] for ref in section_refs), floor=58),
        ...
        if extraction.summary.serviceClues:
            section_items.append(
                _brief_section_recommendation(
                    ...
                    confidence=self._brief_confidence(extraction.confidenceScore, 72, *(ref["confidence"] for ref in section_refs[:2]), floor=52),
        ...
        if extraction.brandAssetCues:
            section_items.append(
                _brief_section_recommendation(
                    ...
                    confidence=self._brief_confidence(*(cue["confidence"] for cue in asset_cues), floor=54),
                    references=self._field_references(extraction=extraction, asset_cues=asset_cues, include_assets=True, limit=3),
                )
            )
        if extraction.summary.ctaClues:
            section_items.append(
                _brief_section_recommendation(
                    ...
                    confidence=self._brief_confidence(extraction.confidenceScore, 66, *(ref["confidence"] for ref in section_refs[:2]), floor=50),
                    references=section_refs[:2],
                )
            )
        if extraction.gapItems:
            section_items.append(
                _brief_section_recommendation(
                    ...
                    confidence=self._brief_confidence(60, extraction.confidenceScore, floor=45),
                    references=[],
                )
            )
        ...
        for reference in source_refs[:4]:
            proof_points.append(
                _brief_proof_point(
                    ...
                    source_kind="source_backed" if reference["kind"] == "page" else "inferred",
                    inference_label="Captured from public source material." if reference["kind"] == "page" else "Captured from public brand assets.",
        ...
        approval_state: str = "approved" if approved else ("needs_review" if missing_requirements or confidence_score < 70 else "draft")
        ...
        company_summary_value = _apply_override(company_summary_value, patch.companySummary if patch else None)
        value_proposition_value = _apply_override(value_proposition_value, patch.valuePropositionSummary if patch else None)
        audience_value = _apply_override(audience_value, patch.audienceHypothesis if patch else None)
        tone_value = _apply_override(tone_value, patch.toneProfile if patch else None)
        conversion_value = _apply_override(conversion_value, patch.conversionAngle if patch else None)
        hero_value = _apply_override(hero_value, patch.recommendedHero if patch else None)
        review_notes = patch.reviewNotes.strip() if patch and patch.reviewNotes is not None else (previous_brief.get("reviewNotes") if previous_brief else None)
        ...
        if patch is not None:
            ...
            if patch.recommendedSections is not None and not [line.strip() for line in patch.recommendedSections if line and line.strip()]:
        ...
        if patch is not None and patch.recommendedSections is not None:
            section_items = [
                ...
                for title in [line.strip() for line in patch.recommendedSections if line and line.strip()]
            ]
        elif previous_brief and patch is not None:
            section_items = list(previous_brief.get("recommendedSections", []))

        if patch is not None:
            proof_points = list(previous_brief.get("proofPoints", [])) if previous_brief else proof_points
            source_refs = list(previous_brief.get("sourceCitations", [])) if previous_brief else source_refs
            if company_summary_value != (previous_brief.get("companySummary", {}).get("value") if previous_brief else None):
                company_confidence = max(company_confidence, 70)
            if value_proposition_value != (previous_brief.get("valuePropositionSummary", {}).get("value") if previous_brief else None):
                value_proposition_confidence = max(value_proposition_confidence, 70)
            if audience_value != (previous_brief.get("audienceHypothesis", {}).get("value") if previous_brief else None):
                audience_confidence = max(audience_confidence, 68)
            if tone_value != (previous_brief.get("toneProfile", {}).get("value") if previous_brief else None):
                tone_confidence = max(tone_confidence, 68)
            if conversion_value != (previous_brief.get("conversionAngle", {}).get("value") if previous_brief else None):
                conversion_confidence = max(conversion_confidence, 68)
            if hero_value != (previous_brief.get("recommendedHero", {}).get("value") if previous_brief else None):
        ...
        brief = {
            ...
            "companySummary": _brief_text_recommendation(
                ...
                inference_label=company_inference if patch is None else "Operator edited against preserved source evidence.",
            ...
            "valuePropositionSummary": _brief_text_recommendation(
                ...
                inference_label="Summarized directly from the public positioning and service language." if value_proposition_signal else "Inferred from the lead record and sparse crawl signals.",
            ...
            "audienceHypothesis": _brief_text_recommendation(
                ...
                source_kind="inferred",
                inference_label="Inferred from audience language in the public crawl." if patch is None else "Operator edited against preserved source evidence.",
            ...
            "toneProfile": _brief_text_recommendation(
                ...
                inference_label="Inferred from CTA and brand cues." if patch is None else "Operator edited against preserved source evidence.",
            ...
            "conversionAngle": _brief_text_recommendation(
                ...
                source_kind="inferred",
                inference_label="Inferred from CTA patterns and service language." if patch is None else "Operator edited against preserved source evidence.",
            ...
            "recommendedHero": _brief_text_recommendation(
                value=hero_value,
                source_kind="inferred" if patch is None else "inferred",
                inference_label="Recommended from source signals before generation." if patch is None else "Operator edited against preserved source evidence.",
        ...
        if patch is not None and previous_brief is not None:
            ...
            brief["sourceExtractionVersion"] = int(previous_brief.get("sourceExtractionVersion", extraction.version))
            brief["proofPoints"] = list(previous_brief.get("proofPoints", []))
            brief["sourceCitations"] = list(previous_brief.get("sourceCitations", []))
            brief["brandAssetProvenance"] = list(previous_brief.get("brandAssetProvenance", []))
    ...
    async def create_brief(self, lead_id: str) -> SiteBrief | None:
        ...
        doc = self._build_brief_doc(lead=lead, extraction=extraction, previous_brief=previous_brief)
        database = get_database()
        if database is None:
            async with self._memory_lock:
    ...
    async def update_brief(self, lead_id: str, patch: SiteBriefPatchRequest) -> SiteBrief | None:
        await self._maybe_ensure_indexes()
        lead = await self.get_lead(lead_id)
        if lead is None:
            return None
        ...
        if extraction is None or extraction.version <= 0:
            raise ValueError("brief_requires_extraction")
        doc = self._build_brief_doc(lead=lead, extraction=extraction, previous_brief=previous_brief, patch=patch)
        database = get_database()
        if database is None:
            async with self._memory_lock:
                self._briefs.setdefault(lead_id, []).append(doc)
    ...
    async def approve_brief(self, lead_id: str, approved_by: str) -> SiteBrief | None:
        ...
        doc["sourceExtractionVersion"] = int(previous_brief.get("sourceExtractionVersion", extraction.version))
    ...
    async def start_extraction(self, lead_id: str, *, refresh: bool = False) -> ExtractionJobResponse | None:
        await self._maybe_ensure_indexes()
        database = get_database()
        lead = await self.get_lead(lead_id)
        if lead is None:
        ...
        job = await self._create_job(
            ...
            metadata={"mode": "refresh" if refresh else "start", "leadWebsiteUrl": lead.websiteUrl},
        )

        await self._update_job(job.id, progress=20, step="Crawling homepage and sitemap")
        crawl_data = crawl_website(lead.websiteUrl, lead_company_name=lead.companyName)
        ...
        if database is None:
            async with self._memory_lock:
                ...
                final_status = "failed" if crawl_data["crawlStatus"] == "failed" else "completed"
                self._jobs[job.id]["progress"] = 100
                self._jobs[job.id]["status"] = final_status
                self._jobs[job.id]["step"] = "Extraction failed" if final_status == "failed" else ("Extraction complete" if not crawl_data["gapItems"] else "Extraction complete with gaps")
                self._jobs[job.id]["errorMessage"] = crawl_data["errors"][0] if final_status == "failed" and crawl_data["errors"] else None
                self._jobs[job.id]["finishedAt"] = now
                self._jobs[job.id]["updatedAt"] = now
                return ExtractionJobResponse(job=_job_doc_to_summary(self._jobs[job.id]), extraction=self._extraction_doc_to_snapshot(doc))

        await database["site_extractions"].insert_one(doc)
        await database["leads"].update_one(
            {"id": lead_id},
            {"$set": {"latestExtractionId": doc["id"], "detectedWebsiteUrl": doc["detectedWebsiteUrl"], "updatedAt": now}},
        )

        final_status = "failed" if crawl_data["crawlStatus"] == "failed" else "completed"
        final_step = "Extraction failed" if final_status == "failed" else ("Extraction complete" if not crawl_data["gapItems"] else "Extraction complete with gaps")
        await self._update_job(
            job.id,
            status=final_status,
            progress=100,
            ...
            finished=True,
            lead_ids=[lead_id],
            error_message=crawl_data["errors"][0] if final_status == "failed" and crawl_data["errors"] else None,
        )
        completed_job = await self.get_job(job.id)
        if completed_job is None:
            raise RuntimeError("extraction_job_missing")
        return ExtractionJobResponse(job=completed_job, extraction=self._extraction_doc_to_snapshot(doc))
