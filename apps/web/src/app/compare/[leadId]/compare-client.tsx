...
type SiteVariant = { id: string; variantLabel: string | null; variantPosition: number; optionNumber: number; previewSlug: string; previewUrl: string; screenshotUrl: string };
...
export function CompareClient({ variants, companyName }: { variants: SiteVariant[]; companyName: string }) {
  ...
  const activeVariant = variants.find((variant) => variant.id === active) ?? variants[0];
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-zinc-100 sm:px-6">
    ...
    <div className="mx-auto max-w-6xl">{variants.length > 1 ? <div className="mb-5 flex flex-wrap gap-2">{variants.map((variant) => <button key={variant.id} onClick={() => setActive(variant.id)} className={`rounded-full px-3 py-2 text-sm ${active === variant.id ? "bg-yellow-500 text-slate-900" : "bg-white/10 text-zinc-300"}`}>Option {variant.optionNumber}</button>)}</div> : null}
      <div className="grid gap-5 sm:grid-cols-2">{variants.map((variant) => <article key={variant.id} className={`overflow-hidden rounded-2xl border ${active === variant.id ? "border-yellow-500/50" : "border-white/10"}`}><div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><div><div className="font-semibold">Option {variant.optionNumber}</div>{variant.variantLabel ? <div className="text-xs text-zinc-400">{variant.variantLabel}</div> : null}</div><a href={variant.previewUrl || `/st/${variant.previewSlug}`} target="_blank" rel="noreferrer" className="text-xs text-yellow-400">Open ↗</a></div><div className="aspect-[4/3] bg-slate-900">{variant.previewUrl ? <iframe src={variant.previewUrl} title={`Option ${variant.optionNumber}`} className="h-full w-full border-0" loading="lazy" sandbox="allow-scripts allow-same-origin allow-forms" /> : <div className="flex h-full items-center justify-center text-sm text-zinc-500">Preview unavailable</div>}</div></article>)}</div>
      {activeVariant ? <div className="mt-6 text-center"><a href={activeVariant.previewUrl || `/st/${activeVariant.previewSlug}`} target="_blank" rel="noreferrer" className="inline-flex rounded-lg bg-yellow-500 px-5 py-3 text-sm font-semibold text-slate-900">Open selected option full screen ↗</a></div> : null}
    </div>
  </main>;
}
